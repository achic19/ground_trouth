    ...
    def calculate_speed(self, join_df, all=False, new_path=''):
        ...
        for index, row in rel_links.iterrows():
            if row['CLOSETS'] - row['LASTDISCOTS'] > 0:
                rel_links.at[index, 'net_link'] = rel_links.at[index, 'Shape_Length']
                rel_links.at[index, 'std_net_link'] = my_dict_std[row['via_to']]
                time_to_link_pos = (row['LASTDISCOTS'] - row['OPENTS'])
                dt = row['CLOSETS'] + time_to_link_pos - row['LASTDISCOTS']
                rel_links.at[index, 'speed'] = my_dict[row['via_to']] / dt
...
def compare_results(data_path, user_type, workspace_csv_progress, confusion_matrix, k_s=0.9):
    """
    ...
    if user_type == 'ped':
        ...
        our_result = data.loc[data['speed'] < 1.5]
...
if __name__ == '__main__':
    ...
    ran_all_def = ['no']
    # Control whether net length should be calculated or not: the options are :'calculate speed','calculate_avg speed', 'compare results','net length'
    start_to_run = 'calculate speed'
    ...
    if start_to_run != 'compare results':
    ...
    list_matrices = []
    for k_s in range(5, 21):
        k_s = k_s / 10
        print(k_s)
        confusion_matrix = {'TP': 0, 'FP': 0, 'FN': 0, 'TN': 0}
        compare_results(path_ped, 'ped', path_to_save, confusion_matrix, k_s)
        compare_results(path_car, 'car', path_to_save, confusion_matrix, k_s)
